...
public:
  ...
  inline String( const char* left, const char* right )
  {
    value_.append(left);
    value_.append(right);
  };
   ...
   * @deprecated
   */
  inline int find( int begin, const char* target )
  {
    const std::string str(target);
    return BaseStringTpl::find( begin, str);
  }
  ...
  {
    return strtoul( value_.c_str(), NULL, 0 );
  }
  inline double dblval() const
  {
    return strtod( value_.c_str(), NULL );
  }
