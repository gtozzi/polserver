...
public:
  ...
  inline explicit Unicode( const UnicodeString& str, UnicodeString::size_type pos, UnicodeString::size_type n ) : BaseStringTpl( str, pos, n ) {}
  inline explicit Unicode( const Unicode& left, const Unicode& right ) : BaseStringTpl( left, right ) {}
  ...
  {
    return wcstoul( value_.asWcharArray(), NULL, 0 );
  }
  inline double dblval() const
  {
    return wcstod( value_.asWcharArray(), NULL );
  }

  inline virtual std::string getStringRep() const POL_OVERRIDE
  {
