...
public:
   ...
   */
  explicit BaseStringTpl( const V& str, V_size_type pos, V_size_type n ) : BaseString( T ), value_( str, pos, n ) {}
        ...
        while ( ( tag_start_pos = value_.find( '{', str_pos ) ) != V::npos )
        {
          if ( ( tag_stop_pos = value_.find( '}', tag_start_pos ) ) != V::npos )
          {
            *result.get() += value_.substr( str_pos, tag_start_pos - str_pos );
            ...
            V tag_body = value_.substr( tag_start_pos + 1, ( tag_stop_pos - tag_start_pos ) - 1 );
            ...
            {
              *result.get() += V("<invalid index: #") + V(boost::lexical_cast<std::string>( tag_param_idx + 1 )) + V(">");
              continue;
            }
            ...
            {
              // accesing object
              BObjectRef obj_member = imp->get_member( static_cast<std::string>(prop_name).c_str() );
