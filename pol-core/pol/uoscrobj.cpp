...
BObjectImp* Character::get_script_member_id( const int id ) const
{
  ...
  case MBR_TRADING_WITH:
    ...
    break;
  case MBR_CLIENTTYPE:
    return new BLong( client != NULL ? client->ClientType : 0 );
    break;
  case MBR_CURSOR:
    if ( client != NULL )
  ...
  case MBR_UO_EXPANSION_CLIENT:
    return new BLong( client != NULL ? client->UOExpansionFlagClient : 0 );
    break;
...
SpeechEvent::SpeechEvent( Mobile::Character* speaker, const char* speech, const char* texttype )
{
  addMember( "type", new BLong( Core::EVID_SPOKE ) );
  addMember( "source", new Module::EOfflineCharacterRefObjImp( speaker ) );
  addMember( "text", new String( speech ) );
